...
var url = require('./url');
var EggRanking = require('./eggRanking');
...
var port = 443;
var redirectUri = 'https://limily.com';
...
var authMiddleware = function(req, res, next) {
  ...
  } else if (urlPathname === '/' || urlPathname === '/auth' || urlPathname === '/score') {
    next();
  } else {
    res.send(401);
  }
...
app.get('/stars', function(req, res) {
  req.msr.getStars(req.query, function(err, data, response) {
    if (err) {
      console.error(err);
      req.msrCookie.reset();
      res.send(500);
...
app.post('/score', function(req, res) {
  new EggRanking().rank(req.body, function(err, data) {
    if (err) {
      console.error(err);
      res.send(500);
    } else {
...
var server = https.createServer(secret.HTTPS_OPTIONS, app).listen(port, function() {
  ...
  log('App server listening to port ' + port);
});
...
var gracefulExit = function() {
  ...
  server.close(function() {
    ...
    EggRanking.shutdown(); // close redis
    process.exit();
  });
