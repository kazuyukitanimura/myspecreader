...
Feedly.prototype._createResponseHandler = function(callback) {
  return function(error, data, response) {
    if (!callback) {
      ...
      var encoding = response.headers['content-encoding']; // incoming header is lower-cased
      ...
      if (!decoders[encoding]) {
        decoders[encoding] = function(data, callback) {
          callback(null, data);
        };
      }
      decoders[encoding](data, function(err, data) {
        if (err) {
          return callback(err, data, reponse);
        }
        try {
