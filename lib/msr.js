...
var tokenize = function(text) {
  ...
  for (var i = tokens.length; i--;) {
    ...
    if (token.length < 3 || ignores.indexOf(token) !== - 1) {
...
Msr.prototype._initScw = function(options) {
  ...
  if (userId && (!this.scwPromise || ! this.scwPromise.scw || options.forceScw)) {
    this.scwPromise = new ScwPromise(userId);
  }
};
    ...
    this.getStreams(options, function(err, data, response) {
      if (!err) {
        ...
        items.sort(function(a, b) {
          //return CATEGORY_LOOKUP[b.estCategory] - CATEGORY_LOOKUP[a.estCategory] || b.must - a.must || b.featureVector.engagement - a.featureVector.engagement || a.featureVector.ago - b.featureVector.ago;
          return b.must - a.must || b.score - a.score || b.featureVector.engagement - a.featureVector.engagement || a.featureVector.ago - b.featureVector.ago;
        });
        for (i = items.length; i--;) {
          item = items[i];
          console.log(item.title,
          /*item.estCategory,*/
          item.score, scw.calcScores(item.featureVector));
          //delete item.estCategory;
          delete item.scores;
          delete item.score;
          delete item.must;
        }
 ...
 * @params callback {Function}
 */
Msr.prototype.getReads = function(options, callback) {
  if (!options) {
    options = {};
    callback = function() {};
  }
  if (!callback) {
    callback(new Error('Msr.getReads Error'));
    return;
  }
  options.globalId = ID_READ;
  this.getStreams(options, function(err, data, response) {
    if (!err) {
      var items = data.items;
      for (var i = items.length; i--;) {
