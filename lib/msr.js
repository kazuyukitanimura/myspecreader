    ...
    this.getStreams(options, function(err, data, response) {
      if (!err) {
        ...
        items.sort(function(a, b) {
          ...
          return b.must - a.must || (b.scores.viewSummary + b.scores.viewOriginal) - (a.scores.viewSummary + a.scores.viewOriginal) || b.featureVector.engagement - a.featureVector.engagement || a.featureVector.ago - b.featureVector.ago;
        });
        for (i = items.length; i--;) {
          item = items[i];
          console.log(item.title, /*item.estCategory,*/ scw.calcScores(item.featureVector));
