    ...
    this.getStreams(options, function(err, data, response) {
      if (!err) {
        ...
        for (var i = items.length; i--;) {
          ...
          var score = max(scores.viewSummary, 0.0) + max(scores.viewOriginal, 0.0) + max(scores.star, 0.0);// + scw.covarianceMatrix.pass.getOrDefault(s);
          scoreMembers.push(score);
          scoreMembers.push(item.id);
          if (scoreMembers.length > 512) {
