...
var createFeatureVector = function(item) {
  ...
  if (isNumber(item.engagement)) {
    featureVector.engagement = item.engagement;
  }
...
// assume the length of array is 2 to the n
var takeTop = function(array, comp, index) {
  for (var i = index; i-- > 0;) {
    var tmpArray = [];
    for (var j = array.length; j-- > 1;) {
      var a = array[j];
      var b = array[--j];
      tmpArray[j >> 1] = comp(a, b) <= 0 ? a: b;
    }
    array = tmpArray;
  }
    ...
    this.getStreams(options, function(err, data, response) {
      if (!err) {
        ...
        for (var i = items.length; i--;) {
          ...
          item.score = max(item.scores.viewSummary, 0.0) + max(item.scores.viewOriginal, 0.0) + max(item.scores.star, 0.0);
        ...
        if (items.length === 2048) {
          items = data.items = takeTop(items, sortComp, 3);
        } else {
          items = data.items = items.sort(sortComp).slice(0, 256);
        }
