...
var createFeatureVector = function(item) {
  ...
  if (isNumber(item.engagementRate)) {
    ...
    featureVector.engagementRate = 8.0;
  }
    ...
    this.getStreams(options, function(err, data, response) {
      if (!err) {
        ...
        for (var i = items.length; i--;) {
          ...
          console.log(scw.covarianceMatrix.viewSummary[s], scw.covarianceMatrix.viewOriginal[s], scw.covarianceMatrix.star[s]);
          var score = (max(scores.viewSummary, 0.0) + max(scores.viewOriginal, 0.0) + max(scores.star, 0.0));
