...
Msr.prototype.updateRanking = function(options) {
  ...
  if (!callback) {
    callback(new Error('Msr.updateRanking Error'));
    return;
  }
    ...
    this.getStreams(options, function(err, data, response) {
      if (!err) {
        ...
        for (var i = items.length; i--;) {
          ...
          var score = item.score = (max(scores.viewSummary, 0.0) + max(scores.viewOriginal, 0.0) + max(scores.star, 0.0));
        ...
        for (i = items.length; i--;) {
          item = items[i];
          console.log(item.title, item.featureVector.engagementRate, item.score, item.scores);
        }
...
Msr.prototype.getRecommends = function(options, callback) {
  ...
  this.ranking.first(options.count || 128).then(function(ids) {
    this.getEntries(ids, function(err, data, response) {
      if (!err) {
        var items = data; // do we need to sort again?
        var item;
        for (var i = items.length; i--;) {
          item = items[i];
