...
var sortUnexploredComp = function(a, b) {
  return b.unexploredScore - a.unexploredScore;
};
    ...
    this.getStreams(options, function(err, data, response) {
      if (!err) {
        ...
        for (var i = items.length; i--;) {
          ...
          var scores = item.scores = scw.calcScores(featureVector);
          item.score = (max(scores.viewSummary, 0.0) + max(scores.viewOriginal, 0.0) + max(scores.star, 0.0)) || max(max(scores.viewSummary, scores.viewOriginal), scores.star);
          item.unexploredScore = scw.weightMatrix.countDefault();
        }
        items.sort(sortComp);
        var restItems = items.slice(64);
        items = data.items = items.slice(0, 64).concat(restItems.sort(sortUnexploredComp).slice(0, 64));
        for (i = items.length; i--;) {
          item = items[i];
          console.log(item.title, item.featureVector.engagementRate, item.score, item.unexploredScore, item.scores); // TODO delete this after debugging
          item.scores = undefined;
          item.score = undefined;
          item.unexploredScore = undefined;
          item.must = undefined;
        }
      }
