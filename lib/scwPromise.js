...
Scw.prototype.train = function(data, callback) {
  ...
  // extract only changed features
  var keys = {};
  for (var i = data.length; i--;) {
    var poses = Object.keys(data[i].featureVector);
  ...
  redis.saveMatricies(this.userId, CATEGORIES, fields, this.covarianceMatrix, this.weightMatrix, callback || function() {});
  redis.saveMatricies(GLOBAL_USER_ID, CATEGORIES, fields, gScw.covarianceMatrix, gScw.weightMatrix, function() {});
};
    ...
    redis.loadMatricies(userId, CATEGORIES, function(err, scwOptions) {
      if (err) {
        ...
        if (!Object.keys(scwOptions.covarianceMatrix[NON_CATEGORY]).length) {
          scwOptions.weightMatrix = gScw.weightMatrix; // copy a global weightMatrix if scwOptions is empty but not covarianceMatrix
        }
